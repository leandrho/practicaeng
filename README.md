...
### Verb Tenses practice (Present, Past, Future and Conditionals)

Las tarjetas **Present**, **Past**, **Future** y **Conditionals** del grupo `Grammar practice` abren `/verb-tenses/present`, `/verb-tenses/past`, `/verb-tenses/future` y `/verb-tenses/conditionals`. Son prácticas puras sin teoría: gap-fill con **Check**, contexto estilo libro y **Reveal** con modelo, explicación y traducción. El encabezado de cada archivo en `data/verb-tenses-*.md` debe coincidir con el catálogo de `src/domain/verb-tenses.ts`.

Cada página ofrece **Filters & Order**: se puede filtrar por las formas específicas del ejercicio y ordenar de forma **Shuffled** o **Ordered**. El orden inicial es **Shuffled**. Si un ejercicio practica más de una forma, aparece en cada filtro correspondiente; al revelar la respuesta, la tarjeta muestra todas sus formas separadas por ` + `.

Present usa `Present Simple`, `Present Progressive`, `Present Perfect Simple` y `Present Perfect Progressive`; Past usa `Past Simple`, `Past Progressive`, `Past Perfect Simple` y `Past Perfect Progressive`. Cada forma específica requiere al menos 10 ejercicios y cada sección al menos 50.

Future cubre `Will`, `Be going to`, `Present Progressive (future arrangement)`, `Present Simple (timetable)`, `Future Progressive`, `Future Perfect Simple`, `Be about to` y `Be due to`. Cada forma requiere al menos 6 ejercicios, salvo `Be due to`, que requiere 4; el mínimo total es 50. El progresivo de arreglo incluye referencia temporal futura, el simple de horario presenta un horario fijado por un tercero, y about to o due to llevan verbo base sin -ing.

Conditionals cubre `Conditional Zero`, `Conditional Type 1`, `Conditional Type 2`, `Conditional Type 3`, `Past Perfect → would + base (present result)` y `Past Simple → would have + past participle (past result)`. Los cuatro tipos estándar requieren al menos 10 ejercicios; cada estructura de resultado cruzado, al menos 5; el mínimo total es 50.

Cada ejercicio es un bullet de una sola línea con el separador literal ` --- `:

```md
## Present
- **Forms:** Present Simple --- **Prompt (EN):** Complete the routine: she / walk to school every day. --- **Sentence (EN):** She ____ to school every day. --- **Answers:** walks --- **Model (EN):** She walks to school every day. --- **Explanation (ES):** El *present simple* expresa hábitos y la tercera persona lleva -s. --- **Translation (ES):** Ella camina a la escuela todos los días. --- **Context (EN):** "Do you walk to school?" "Yes, I walk there every day with my brother." --- **Context source:** Everyday conversation
```

Convenciones:

- `Forms` contiene una o más formas de la lista cerrada correspondiente a la sección, separadas por comas; el parser rechaza valores vacíos, repetidos o ajenos a esa sección.
