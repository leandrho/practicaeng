...
### Verb Tenses practice (Presente, Pasado, Futuros y Condicionales)
...
La tarjeta **Pasado** del mismo grupo abre `/verb-tenses/past` con el mismo formato combinado y sin teoría. El contenido se edita en `data/verb-tenses-past.md`, cuyo encabezado `## Pasado` debe coincidir con el catálogo. La sección tiene 55 ejercicios, con 11 por forma (`Past Simple`, `Past Progressive`, `Past Perfect Simple`, `Past Perfect Progressive`, `Mixed`); el mínimo exigido es el mismo: 50 en total y 10 por forma.

La tarjeta **Futuros** del mismo grupo abre `/verb-tenses/future` con el mismo formato combinado y sin teoría. El contenido se edita en `data/verb-tenses-future.md`, cuyo encabezado `## Futuros` debe coincidir con el catálogo. La sección tiene 50 ejercicios, con 6 por forma en `Will`, `Be going to`, `Present Progressive (future arrangement)`, `Present Simple (timetable)`, `Future Progressive`, `Future Perfect Simple` y `Be about to / Be due to`, más 8 en `Mixed`; el mínimo exigido es 50 en total, 6 por forma y 8 en mixtos. El progresivo de arreglo incluye referencia temporal futura en la oración, el simple de horario presenta un horario fijado por un tercero, y about to o due to llevan verbo base sin -ing.

La tarjeta **Condicionales** del mismo grupo abre `/verb-tenses/conditionals` con el mismo formato combinado y sin teoría. El contenido se edita en `data/verb-tenses-conditionals.md`, cuyo encabezado `## Condicionales` debe coincidir con el catálogo. La sección tiene 50 ejercicios, con 10 por forma (`Conditional Zero`, `Conditional Type 1`, `Conditional Type 2`, `Conditional Type 3`, `Mixed`); el mínimo exigido es 50 en total y 10 por forma. En condicionales el hueco puede estar en la prótasis o en la apódosis y el resto de la oración fija el tipo; en `Mixed` el prompt describe la situación sin nombrar el tipo.
